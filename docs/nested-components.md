...
```js
const CONTAINER_TAGS = ['div','section','header','footer','main','aside',
                        'nav','form','ul','ol','table','tbody','thead',
                        'tr','dl'];
function isContainer(el) { return CONTAINER_TAGS.includes(el.tagName.toLowerCase()); }
```
...
- Use event delegation on `canvas` rather than binding per element.
  ...
  is a separate concern for the *destination*.
